...
HRESULT CreatePixelShaders(IDirect3DDevice9* pd3dDevice)
{
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("TexAtlasHiLite.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("AbsDifference.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("AbsDifferenceMag.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("Wrap.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("WrapAbsDiff.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("WrapAbsDiffMag.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("Clamp.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("ClampAbsDiff.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("ClampAbsDiffMag.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("Mirror.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("MirrorAbsDiff.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
    ...
    hr = D3DXAssembleShaderFromFile(GetFilePath::GetFilePath(TEXT("MirrorAbsDiffMag.ps")).c_str(), NULL, NULL, 0, &pShaderToken, &pErrorMsgs);
    if (hr != S_OK)
    {
        MessageBox( NULL, TEXT("Unable to load pixel shader."), g_strWindowTitle, MB_ICONERROR|MB_OK );
        return hr;
...
void LoadCurrentEntry(IDirect3DDevice9* pd3dDevice)
{
        ...
        || AtlasIndex != g_CurrentAtlasIndex || AtlasIndex == -1)
    {
        ...
        switch (info.ResourceType)
        {
            case D3DRTYPE_TEXTURE:
                hr = D3DXCreateTextureFromFileEx( pd3dDevice, GetFilePath::GetFilePath(TAtlasname).c_str(), D3DX_DEFAULT, D3DX_DEFAULT, info.MipLevels, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, &info, NULL, &g_pCurrentTextureAtlas);
                break;
            case D3DRTYPE_VOLUMETEXTURE:
                hr = D3DXCreateVolumeTextureFromFileEx( pd3dDevice, GetFilePath::GetFilePath(TAtlasname).c_str(), D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, info.MipLevels, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, &info, NULL, &g_pCurrentVolumeAtlas);
                break;
            default:
                hr = E_FAIL;
                break;
    ...
    hr = D3DXCreateTextureFromFileEx( pd3dDevice, GetFilePath::GetFilePath(lpszW).c_str(), D3DX_DEFAULT, D3DX_DEFAULT, kMipLevels, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, &info, NULL, &g_pCurrentTexture);
    if ( hr != D3D_OK )
...
void RenderTopLeftQuadrant(D3DVIEWPORT9 &viewport, IDirect3DDevice9* pd3dDevice)
{
    ...
    _stprintf(string, TEXT("Original Texture\n(%dx%d, %d Mipmap(s))"), mCurrentTextureDesc.Width, mCurrentTextureDesc.Height, mips);
...
void RenderBottomRightQuadrant(D3DVIEWPORT9 &viewport, IDirect3DDevice9* pd3dDevice, float fElapsedTime)
{
    ...
    D3DXMatrixScaling(&matWorld, 1.4f, 1.4f, 1.0f);
    pd3dDevice->SetTransform(D3DTS_WORLD, &matWorld);
    ...
    if (g_pCurrentTextureAtlas)
    {
        ...
        _stprintf(string, TEXT("Complete Texture Atlas\n(%dx%d, %d Mipmap(s))"), descwidth, descheight, mips);
    }
    else
    {
        _stprintf(string, TEXT("Volume Texture Atlas Slice\n(%dx%dx%d, %d Mipmap(s))"), descwidth, descheight, mCurrentVolumeAtlasDesc.Depth, 1);
    }
