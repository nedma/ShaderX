...
//@ SDK version is 31 for both 9.0b Summer 2003 update AND  DX9.0c Summer 2004 update
// That is unfortunate, given the differences between the two.
//
//#if D3D_SDK_VERSION == 31
// if __D3DX9XOF_H__ is defined, then we are dealing with DX9.0c Summer 2004 update
#ifdef __D3DX9XOF_H__
	#include <dxfile.h>		// IDirectXFileData
	// Need to use LPD3DXFILEDATA for the D3DXLoadMeshFromXof function in the Summer 2004 SDK update
	#ifndef NV_LPD3DXFILEDATA
	#define NV_LPD3DXFILEDATA		LPD3DXFILEDATA
	#endif
	#define NV_USEDX90C
#else
	#ifndef NV_LPD3DXFILEDATA
	#define NV_LPD3DXFILEDATA		LPDIRECTXFILEDATA
	#endif
#endif
...
public:
	...
	HRESULT Create( LPDIRECT3DDEVICE9 pd3dDevice, NV_LPD3DXFILEDATA pFileData );
    HRESULT Destroy();
...
{
    HRESULT LoadMesh( LPDIRECT3DDEVICE9 pd3dDevice, NV_LPD3DXFILEDATA pFileData, 
                      CD3DFrame* pParentFrame );
    HRESULT LoadFrame( LPDIRECT3DDEVICE9 pd3dDevice, NV_LPD3DXFILEDATA pFileData,    
                       CD3DFrame* pParentFrame );
public:
    HRESULT Create( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR strFilename );
