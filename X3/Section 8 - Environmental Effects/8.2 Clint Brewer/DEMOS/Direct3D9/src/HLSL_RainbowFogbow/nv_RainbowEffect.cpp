...
HRESULT nv_RainbowEffect::RestoreDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice , const char* dataTextureFile)
{
	...
	std::string path;
	path = GetFilePath::GetFilePath(dataTextureFile, false );
	if(path.empty())
	...
	path = GetFilePath::GetFilePath("Textures\\2D\\rainbow_plot_i_vs_a_diffract_0_90_1024.tga", false );
	if(path.empty())
		return D3DAPPERR_MEDIANOTFOUND;
...
HRESULT nv_RainbowEffect::Create( LPDIRECT3DDEVICE9 pd3dDevice)
{
	...
	//load our fx file
	std::string path;
	path = GetFilePath::GetFilePath("HLSL\\RainbowFogbow.fx", false );
	if(path.empty())
		return D3DAPPERR_MEDIANOTFOUND;
