...
HRESULT NVBScene::LoadTexture(const std::string& name, LPDIRECT3DTEXTURE9& texture, GetFilePathCallback getFilePath, D3DFORMAT format, D3DPOOL pool)
{
	...
	if (name == "")
	{
		tempName = "ruf-wood.tga";
	}
	//std::string fullname = (getFilePath ? (*getFilePath)(name) : name);
	std::string fullname = (getFilePath ? (*getFilePath)(tempName) : tempName);
	...
	for (unsigned int i = 0; i < m_Scene.num_nodes; ++i) {
		...
		if (node->get_type() == nv_node::GEOMETRY) {
			...
			for (unsigned int j = 0; j < model->num_meshes; ++j) {
				...
				Ptr<D3DXVECTOR3[2]> S(d3dMesh.m_NumVertices);
				//Ptr<D3DXVECTOR3[2]> T(d3dMesh.m_VertexHasT ? d3dMesh.m_NumVertices : 0);
				Ptr<D3DXVECTOR3[2]> T(d3dMesh.m_NumVertices);
				...
				for (unsigned int k = 0; k < d3dMesh.m_NumVertices; ++k) {
					...
					{
						vert.m_Texture.x = 0.0f;
						vert.m_Texture.y = 0.0f;
					}
