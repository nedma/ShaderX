...
//-----------------------------------------------------------------------------
D3DXMATRIX D3DUtil_GetCubeMapViewMatrix( DWORD dwFace, const D3DXVECTOR3& eyePt )
{
    ...
    D3DXMATRIXA16 mView;
    D3DXMatrixLookAtLH( &mView, &eyePt, &(eyePt+vLookDir), &vUpDir );
    return mView;
}
...
End:
    ...
    if( hdcMask != NULL )
        DeleteDC( hdcMask );
    DXSAFE_DELETE_ARRAY( pcrArrayColor );
    DXSAFE_DELETE_ARRAY( pcrArrayMask );
    DXSAFE_RELEASE( pCursorSurface );
    return hr;
}
