...
{
    m_strWindowTitle = _T("MeshDeform");
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
	...
	switch(m_bEffect){
		...
		break;
		case 3:
		hr = DXUtil_FindMediaFileCb( str, sizeof(str), TEXT("HLSL_Taper_Twist.fx") );
		break;
    }
...
/* Optimizing is supressed, to conserve Default Normals !
...
HRESULT CMyD3DApplication::BuildTubeMesh()
{
   ...
   // SAFE_RELEASE( m_pTubeMesh );
    ...
    for (z=0; z < (int)(m_m)-1; z++)
    {
        for (int x=0; x <(int) (m_n)-1; x++)
        {
            v[k].p  = getCirclePos(x*dX,z*dZ,width,0.2);

            k++;

            v[k].p  = getCirclePos(x*dX,(z+1)*dZ,width,0.2);

		    k++;
			
			v[k].p  = getCirclePos((x+1)*dX,(z+1)*dZ,width,0.2);
			...
			v[k].p  = getCirclePos((x+1)*dX,(z)*dZ,width,0.2);
