...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
    ...
    if( SUCCEEDED(hr) )
    {
		// ShaderX3 Code: Attach the probe
		hr = m_Probe.AttachToDevice(m_pd3dDevice,&m_d3dpp);
		if (FAILED(hr))
			return hr;
...
HRESULT CD3DApplication::Reset3DEnvironment()
{
	...
	// ShaderX3 Code: Reset device
	hr = m_Probe.ResetDevice(&m_d3dpp);
	if (FAILED(hr))
		return hr;
...
HRESULT CD3DApplication::Render3DEnvironment()
{
	...
	// ShaderX3 Code: Make sure device is good and running
	hr = m_Probe.ProbeDevice();
	if( D3DERR_DEVICELOST == hr )
	{
		m_bDeviceLost = true;

		// The framework blindly intercepts this return value
		// and tests it for failure. When that is the case, it
		// quits the application directly. Here, we just
		// return success, but raise the m_bDeviceLost flag,
		// which tells the framework to stop rendering, but
		// keep probing the device at constant low-frequency
		// intervals.
		return S_OK;
	}
	if (FAILED(hr))
...
{
	// ShaderX3 Code: Detach off the device
	m_Probe.DetachFromCurrentDevice();
