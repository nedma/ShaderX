...
float AveragedNoise(float x,float y, float z)
{
		...
		float noisev=((noise(x*5,y*5,z*5)*(5-x*5)*(5-y*5)*(5-z*5))+
...
const D3DXVECTOR3* pTexelSize, LPVOID pData)
{
	...
	//	*pOut = D3DXVECTOR4(1,1,1,1-alpha*3);	
		...
		float dxnoisev=AveragedNoise(pTexCoord->x+.1,pTexCoord->y,pTexCoord->z)-AveragedNoise(pTexCoord->x,pTexCoord->y,pTexCoord->z)/.1;
		float dynoisev=AveragedNoise(pTexCoord->x,pTexCoord->y+.1,pTexCoord->z)-AveragedNoise(pTexCoord->x,pTexCoord->y,pTexCoord->z)/.1;
		float dznoisev=AveragedNoise(pTexCoord->x,pTexCoord->y,pTexCoord->z+.1)-AveragedNoise(pTexCoord->x,pTexCoord->y,pTexCoord->z)/.1;		
...
HRESULT CAppForm::RestoreDeviceObjects()
{
    ...
    HRESULT hr;
	pShader=NULL;
	hr = D3DXCompileShaderFromFile ("VolumeClouds.cg",NULL,NULL,"main","ps_2_0",0,&pShader,0,&m_pConstantTable );    
	if( hr!=D3D_OK )   
	{        
	...
	return hr;   
     ...
     hr = m_pd3dDevice->CreatePixelShader((DWORD*)pShader->GetBufferPointer(), &pPixelShader ); 
	 Density=m_pConstantTable->GetConstantByName(NULL,"Blend");
	 NoiseScale1=m_pConstantTable->GetConstantByName(NULL,"NoiseScale1");
	 NoiseScale2=m_pConstantTable->GetConstantByName(NULL,"NoiseScale2");
	 BlendImplict=m_pConstantTable->GetConstantByName(NULL,"ImplictBlend");
	 SAFE_RELEASE(pShader);    
	 /*if(hr!=D3D_OK)    
	 {    
...
HRESULT CAppForm::Render()
{
    ...
    // Begin the scene
	m_pConstantTable->SetFloat(m_pd3dDevice,Density,m_Density);
	m_pConstantTable->SetFloat(m_pd3dDevice,NoiseScale1,m_Noise1);
	m_pConstantTable->SetFloat(m_pd3dDevice,NoiseScale2,m_Noise2);
	m_pConstantTable->SetFloat(m_pd3dDevice,BlendImplict,m_BlendImplict);
...
void CAppForm::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MinMaxFloat(pDX, m_Density, 0.f, 1.f);
	DDX_Text(pDX, IDC_EDIT2, m_Noise1);
	DDV_MinMaxFloat(pDX, m_Noise1, 1.f, 10.f);
	DDX_Text(pDX, IDC_EDIT3, m_Noise2);
	DDV_MinMaxFloat(pDX, m_Noise2, 1.f, 10.f);
	DDX_Text(pDX, IDC_EDIT4, m_BlendImplict);
	DDV_MinMaxFloat(pDX, m_BlendImplict, 0.f, 1.f);
	//}}AFX_DATA_MAP
}
