...
//  This file is exactly the same as R32FCubemap, except it uses "texCUBEbias" texture lookups,
//  to workaround a Radeon bug where the MIPMAPLODBIAS sampler state is ignored

float    cZBias;                        // constant Z-bias
float    cLODBias;                      // constant LOD bias
...
{
	return compareDepth < texCUBEbias(shadowMap, float4(texcoord,cLODBias)).r;
}
...
float FilteredShadowComparison(samplerCUBE shadowMap, float3 texcoord, float compareDepth)
{
    ...
    //  vectorize the lerps
    shadowDepth[0].x = texCUBEbias(shadowMap, float4(shadowCoord[0],cLODBias) ).r;
    shadowDepth[0].y = texCUBEbias(shadowMap, float4(shadowCoord[2],cLODBias) ).r;
    shadowDepth[0].z = texCUBEbias(shadowMap, float4(shadowCoord[4],cLODBias) ).r;
    shadowDepth[0].w = texCUBEbias(shadowMap, float4(shadowCoord[6],cLODBias) ).r;
    shadowDepth[1].x = texCUBEbias(shadowMap, float4(shadowCoord[1],cLODBias) ).r;
    shadowDepth[1].y = texCUBEbias(shadowMap, float4(shadowCoord[3],cLODBias) ).r;
    shadowDepth[1].z = texCUBEbias(shadowMap, float4(shadowCoord[5],cLODBias) ).r;
    shadowDepth[1].w = texCUBEbias(shadowMap, float4(shadowCoord[7],cLODBias) ).r;
