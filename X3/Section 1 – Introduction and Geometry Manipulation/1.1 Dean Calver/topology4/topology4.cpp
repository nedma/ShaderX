...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"Topology2.fx" ) );
    // If this fails, there should be debug output as to 
    // they the .fx file failed to compile
    V_RETURN( D3DXCreateEffectFromFile( pd3dDevice, str, NULL, NULL, dwShaderFlags, 
                                        NULL, &g_pEffect2, NULL ) );
...
//	V_RETURN( VertexMapCreator::ConvertXIndexToVertexStream( pd3dDevice, g_teapotMesh, g_indexVertexStream ) );
	...
	// create the render target face normal texture

	g_numTriangles = g_teapotMesh->GetNumFaces();
	V_RETURN( VertexMapCreator::CreateRTMap( 
		pd3dDevice, g_teapotMesh, VertexMapCreator::MF_PER_FACE, g_rtFaceNormalTexture,
		g_topoMapSizes ) );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		...
		RenderFaceNormalMap( pd3dDevice );
		...
		V( g_pEffect->SetTexture( "g_faceNormalTopoMap", g_rtFaceNormalTexture ) );
		...
		for( unsigned int i = 0;i < iPasses;i++)
		{
			...
			// for some weird reason geometry instancing REQUIRES indexed primitve's
