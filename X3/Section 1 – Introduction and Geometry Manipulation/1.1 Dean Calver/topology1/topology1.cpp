...
//	V_RETURN( D3DXCreateSphere( pd3dDevice, 1,10,10, &g_teapotMesh, 0 ) );
	...
	V_RETURN( VertexMapCreator::ConvertXIndexToVertexStream(
		pd3dDevice, g_teapotMesh, g_indexVertexStream ) );
	V_RETURN( VertexMapCreator::Create123VertexStream(
		pd3dDevice, g_123VertexStream ) );
	V_RETURN( VertexMapCreator::Create123IndexStream(
		pd3dDevice, g_123IndexStream ) );
	...
	D3DVERTEXELEMENT9 indexDecl[] = 
	{
		...
		{ 1, 0, D3DDECLTYPE_SHORT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 1 },
		D3DDECL_END(),
	};
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		...
		V( g_pEffect->SetFloat( "g_recipTexWidth", 1.f / (float)VertexMapCreator::SIZE_OF_MAP ) );
		V( g_pEffect->SetFloat( "g_recipTexHeight", 1.f / (float)VertexMapCreator::HeightOfMap ) );
		...
		for( unsigned int i = 0;i < iPasses;i++)
		{
			...
			V( pd3dDevice->SetStreamSourceFreq( 0, g_numTriangles | D3DSTREAMSOURCE_INDEXEDDATA ) ); // repeat every 3 vertices
			V( pd3dDevice->SetStreamSource( 1, g_indexVertexStream, 0, sizeof(short) * 4 ) );
			V( pd3dDevice->SetStreamSourceFreq( 1, 1 | D3DSTREAMSOURCE_INSTANCEDATA ) ); // update every 3rd vertex
