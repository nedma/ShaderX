...
//-----------------------------------------------------------------------------
class RainbowFogbowSDKDemo : public CD3DApplication
{
public:
...
private:
	...
	D3DXVECTOR4					m_sunLightDirection;
	D3DXMATRIX					m_worldTransform;
	CD3DMesh*					m_pSkyBoxMesh;
	CD3DMesh*					m_pSkyBoxMoistureMesh;
	CD3DMesh*					m_pTerrainMesh;

	nv_RainbowEffect*			m_pNVRainbowEffect;
	float						m_rainbowDropletRadius;
	float						m_rainbowIntensity;
	LPD3DXEFFECT				m_pObjectEffects;
	D3DXHANDLE					m_hTechniqueRenderObjectsNormal;
	D3DXHANDLE					m_hTechniqueRenderObjectsBlack; 
	D3DXHANDLE					m_hTechniqueRenderSkyBoxMoisture; 

	LPD3DXEFFECT				m_pSkyBoxRainEffect;

	//helper to set proper directories and find paths and such
	HRESULT	LoadMeshHelperFunction(CD3DMesh* mesh, std::string meshFile);

	//helper function to set up matrices for object effects
	void	SetUpMatriceeesForEffect(	D3DXMATRIX* view, 
										D3DXMATRIX* world, 
										D3DXMATRIX* proj);
	void						RenderMoisturePass();
	void						RenderSceneNormally();
	bool						m_VisualizeRenderSteps;

	//sets the viewport to be at x, y and have width of w, and height of h
	void						SetViewPortHelper(DWORD x, DWORD y, DWORD w, DWORD h);

	std::vector<std::string>    m_RainbowDataTextures;
	int							m_currentRainbowDataTexture;
};
