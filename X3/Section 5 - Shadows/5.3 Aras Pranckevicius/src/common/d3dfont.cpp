...
HRESULT CD3DFont::RestoreDeviceObjects()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->GetDirect3D( &pd3d9 ) ) )
    {
        ...
        if( SUCCEEDED( m_pd3dDevice->GetRenderTarget( 0, &pSurf ) ) )
        {
            ...
            DXSAFE_RELEASE( pSurf );
        }
        DXSAFE_RELEASE( pd3d9 );
    }
...
{
    DXSAFE_RELEASE( m_pVB );
    DXSAFE_RELEASE( m_pStateBlockSaved );
    DXSAFE_RELEASE( m_pStateBlockDrawText );
