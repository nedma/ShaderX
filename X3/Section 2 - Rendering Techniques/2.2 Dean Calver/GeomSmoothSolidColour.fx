...
half3	 f3Colour0 = half3(1.0, 1.0, 1.0);
...
PS_OUT GeomSmoothSolidColourPS( PS_IN inp )
{
	...
	outp.Cs0.xyz = f3Colour0.xyz * f3Material.x;
	outp.Cs1.xyz = f3Colour1.xyz * f3Material.y;

	outp.Position.w = 0;					// unused
	outp.Normal.w = f3Material.z;			// Material index for attributes that change infrequently
	outp.Cs0.w = f2DotProductFuncs.x;		// Fa - the dot product shaping function for N.L
	outp.Cs1.w = f2DotProductFuncs.y;		// Fb - the dot product shading funtcion for N.H
...
{
	float4 retv;
	retv.xyz = f3Colour0 * f3Material.x;
	retv.w = f2DotProductFuncs.x;		// Fa - the dot product shaping function for N.L
	return retv;
}
...
{
	float4 retv;
	retv.xyz = f3Colour1 * f3Material.y;
	retv.w = f2DotProductFuncs.y;		// Fb - the dot product shaping function for N.H
	return retv;
}
