...
#define INIT_RANDOM_NUMBER_GENERATOR() 
...
int GenPMain(int minPoints, int maxPoints, vector<Point2> &MainResult)
{
    ...
    while (1) {
        ...
        if (found >= minPoints && found <= maxPoints) {
            ...
            {
                result[i].u = result[i].u * 2.0 - 1.0;
                result[i].v = result[i].v * 2.0 - 1.0;
//                cout << result[i].u << ",\t" << result[i].v << "," << endl;
            }
            MainResult.clear();
            for (unsigned int i = 0; i < result.size(); ++i)
            {
                if (result[i].u * result[i].u + result[i].v * result[i].v < 1.0)
                    MainResult.push_back(result[i]);
                //                cout << result[i].u << ",\t" << result[i].v << "," << endl;
            }
            cerr << "In the disk " << MainResult.size() << endl;
            break;
        }
